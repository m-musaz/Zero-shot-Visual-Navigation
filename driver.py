...
#im.newRandomEpisode(['1LXtFkjw3qL'])
...
while True:
    ...
    for idx, loc in enumerate(locations[1:]):
        ...
        # Draw the first text (index)
        cv2.putText(rgb, str(idx + 1), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 
                    fontScale, TEXT_COLOR, thickness=3)

        # Offset for second text (below the first one)
        text_offset = int(30 * fontScale)  # Adjust the spacing based on scale

        # Draw the second line of text (custom message)
        cv2.putText(rgb, loc.viewpointId, (x, y + text_offset), cv2.FONT_HERSHEY_SIMPLEX, 
                    fontScale * 0.8, TEXT_COLOR, thickness=2)  # Slightly smaller font
